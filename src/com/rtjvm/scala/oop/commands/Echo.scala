...
class Echo(args: Array[String]) extends Command {
  ...
  def doEcho(state: State, contents: String, filename: String, append: Boolean): State = {
    if (filename.contains(Directory.SEPARATOR)) state.setMessage("Echo: filename must not contain separators")
      ...
      val newRoot: Directory = getRootAfterEcho(state.root, state.wd.getAllDirectoriesInPath :+ filename, contents, append)
      if (newRoot == state.root) state.setMessage(s"${filename}: no such file")
      else State(newRoot, newRoot.findDescendant(state.wd.getAllDirectoriesInPath))
    }
  }
